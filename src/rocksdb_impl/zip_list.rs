...
use crate::{read_int, write_int, EndianScalar, LenType, RrError, RtType, BYTES_LEN_TYPE};
...
impl ZipList {
    ...
    pub fn push_right(&mut self, value: &[u8]) {
        self.insert_left(self.len() as RtType, value);
    }

    pub fn insert_left(&mut self, index: RtType, value: &[u8]) -> bool {
        if index == self.len() as RtType {
    ...
    pub fn insert_right(&mut self, index: RtType, value: &[u8]) -> bool {
        let left_index = {
            if self.len() == 0 {
                0
            } else {
    ...
    pub fn insert_value_left(&mut self, pivot: &[u8], value: &[u8]) -> Option<RtType> {
        let mut index = 0;
    ...
    /// 没有找到pivot 返回None
    /// 找到并成功插入，返回插入后的 index
    pub fn insert_value_right(&mut self, pivot: &[u8], value: &[u8]) -> Option<RtType> {
        let mut index = 0;
        let mut it = ZipListIter::new(self);
        let find_value = it.find(|it| {
            index += 1;
    ...
    //返回原来的值
    pub fn set(&mut self, index: RtType, value: &[u8]) -> Option<Vec<u8>> {
        let offset = self.get_offset_index(index as usize);
        if let Some(offset) = offset {
            let node = ZipListNode::from_start(&self.0[offset..]);
            node.as_ref()?;
    ...
    pub fn rem(&mut self, count: RtType, value: &[u8]) -> LenType {
        ...
        if !removes.is_empty() {
            ...
            self.set_len(self.len() - will_remove as LenType);
        }
    ...
    pub fn index<'a>(&'a self, index: RtType) -> Option<&'a [u8]> {
        let offset = self.get_offset_index(index as usize);
        if let Some(offset) = offset {
    ...
    pub fn range(&self, start: RtType, stop: RtType) -> Vec<Vec<u8>> {
    ...
    pub fn count_index(len: RtType, index: RtType) -> RtType {
        {
            if index < 0 {
...
mod test {
    ...
    fn test_zip_list_insert_left() {
        ...
        zip.insert_left(zip.len() as _, &[2, 3]);
        assert_eq!(
            &[2, 0, 0, 0, 1, 0, 1, 1, 0, 2, 0, 2, 3, 2, 0],
            zip.0.as_slice()
        );
