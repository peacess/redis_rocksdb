...
/// 字段名使用 max head存放
pub struct MaxHeapTrans {}

impl<'db> Heap<Transaction<'db, TransactionDB>> for MaxHeapTrans {
    fn pop(&self, t: &Transaction<'db, TransactionDB>, key: &[u8]) -> Result<Option<(Vec<u8>, Vec<u8>)>, RrError> {
    ...
    fn push(&self, t: &Transaction<'db, TransactionDB>, key: &[u8], field: &[u8], value: &[u8]) -> Result<(), RrError> {
        let field_key = make_key(key, field);
