...
impl VecBytes {
    ...
    pub const offset_meta: isize = VecBytes::offset_data + size_of::<u64>() as isize;
    ...
    /// keys已排序
    pub fn init_keys(data: &mut Vec<u8>,offset: isize, keys_: &[&[u8]]) -> VecBytes {
        ...
        for index in 0..keys_.len() {
            keys.insert_with_index::<KeyMetas>(data, keys_[index], Some(index as usize));
        }
        keys
    }
    ...
    pub fn read_from(&mut self, data: &[u8], offset: isize) {
        ...
        unsafe {
            ...
            self.metas_capacity = read_int_ptr(data.as_ptr().offset(VecBytes::offset_data));
        }
    }

    /// 在index处插入key,
    pub fn insert_with_index<T: Metas>(&mut self, data: &mut Vec<u8>, key: &[u8], index: Option<usize>) {
        if self.is_expand() {
            self.expand(data);
        }
        let mut add_bytes = VecBytes::compute_new_bytes(key);
        data.reserve_exact(add_bytes as usize);

        let start = self.offset + VecBytes::offset_data + self.metas_capacity as isize + self.bytes_data as isize;
        unsafe {
        ...
        {
            let mut meta = T::new(data, VecBytes::offset_meta, self.number_keys, (self.metas_capacity as usize / size_of::<MetaKey>()) as i64);
            let index = match index {
                Some(i) => i,
                None => {
                    match meta.search(data.as_slice(), self.offset + VecBytes::offset_meta + self.metas_capacity as isize, key) {
                        Ok(i) => i,
                        Err(i) => i
                    }
                }
    ...
    pub fn inserts<T:Metas>(&mut self, data: &mut Vec<u8>, keys: &[&[u8]]) {
        for key in keys {
            self.insert_with_index::<T>(data, key, None);
        }
    }
...
trait Metas{
    fn new(data: &mut [u8], offset: isize, len: LenType, cap: i64) -> Self;
...
impl Metas for KeyMetas {
    fn new(data: &mut [u8], offset: isize, len: LenType, cap: i64) -> Self {
        unsafe {
            Self {
                data: Vec::from_raw_parts(data.as_mut_ptr().offset(offset) as *mut _, len as usize, cap as usize),
            }
        }
    }
    fn insert(&mut self, index: usize, key_meta: BytesMeta) {
