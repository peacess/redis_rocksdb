use crate::RrError;
use crate::{Bytes, LenType, RtType};
...
pub trait RedisList {
    ...
    // /// 返回len of list
    // fn lpushs<K: Bytes, V: Bytes>(&mut self, key: &K, values: &[&V]) -> Result<RtType, RrError>;
    ...
    fn ltrim<K: Bytes>(&mut self, key: K, start: RtType, stop: RtType) -> Result<RtType, RrError>;
    ...
    // /// 返回len of list
    // fn rpushs<K: Bytes, V: Bytes>(&mut self, key: &K, value: &[&V]) -> Result<RtType, RrError>;
    /// 为已经存在的列表添加值， 添加到尾部
